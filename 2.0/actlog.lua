local MoonLib = Game.MoonLib

---@class ActLog
ActLog = {}
ActLog.version = 0.1
ActLog.config = {}
ActLog.config.events = {
    ---Game Event
    ['Game.Load'] = true,
    ['Game.Start'] = true,
    ['Game.Run'] = false,
...
local Logger = MoonLib.logger:new("ActLog")
local datatab = {}
...
function ActLog:placeholderReplace(eventtab, str)
    ...
    for _ = 1, 10 do
        ...
        elseif i + 1 == j then
            ...
            lastcount = j + 1
            local result = LoadLuaScript('return __ssmod__.'..phstr:gsub("_", "."))(eventtab)
            strtab[#strtab+1] = result
        end
    end
end

local function eventLog(e)
    local param = {}
    local cep = e.CurEventParam
    param.msec = threadpool.env.msec
    param.time = os.time()
    if cep.TriggerByPlayer then
        _, param.TBPName = Player:getNickname(cep.TriggerByPlayer)
        param.TType = '玩家'
    end
    if cep.TriggerByCreature then
        _, param.TBCName = Creature:getActorName(cep.TriggerByCreature)
        param.TType = '生物'
    end
    if cep.EventTargetItemID then
        _, param.ETIName = Item:getItemName(cep.EventTargetItemID)
    end
    if cep.EventTargetBlock then
        _, param.ETBName = Item:getItemName(cep.EventTargetBlock)
        param.ETType = '方块'
    end
    if cep.EventBuffid then
        _, param.EBName = Buff:GetBuffDefName(cep.EventBuffid)
    end
    if cep.EventTargetCreature then
        _, param.ETCName = Creature:getActorName(cep.EventTargetCreature)
        param.ETType = '生物'
    end
    if cep.EventTargetPlayer then
        _, param.ETPName = Player:getNickname(cep.EventTargetPlayer)
        param.ETType = '玩家'
    end
...
local function regevent()
    local count = 0
    for k, v in pairs(ActLog.config.events) do
        if v then
            ScriptSupportEvent:registerEvent(k, eventLog)
            count = count + 1
        end
    end
   ...
   if strs[1] == '/al' or strs[1] == '/actlog' then
        if strs[2] == 'list' then
            ...
            for i = logtotal - page * 10, logtotal - (page - 1) * 10 do
                if datatab[i] then
                    Chat:sendSystemMsg(os.date('[%H:%M:%S] ', datatab[i].ActLog.time)..ActLog:placeholderReplace(datatab[i], ActLog.config.eventlang[datatab[i].msgStr]),uin)
                end
            end
